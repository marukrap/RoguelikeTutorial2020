...
void World::initialize(unsigned int seed, bool placeEntities)
{
	Rng rng(seed);
	m_rngSeed = seed;
...
{
	serialize(os, m_rngSeed);

	m_fov->save(os);
	m_panel->save(os);

	const std::size_t numActors = m_actors.size();
...
{
	deserialize(is, m_rngSeed);
	initialize(m_rngSeed, false);
	...
	m_actors.resize(numActors);
	for (std::size_t i = 0; i < numActors; ++i)
	{
		std::string name;
		deserialize(is, name);
		m_actors[i] = std::make_unique<Actor>(name);
		m_actors[i]->load(is);
	}

	std::size_t playerId;
	deserialize(is, playerId);
	m_player = m_actors[playerId].get();
	m_panel->setPlayer(m_player);
	...
	for (std::size_t i = 0; i < numItems; ++i)
	{
		std::string name;
		deserialize(is, name);
		m_items[i] = std::make_unique<Item>(name);
		m_items[i]->load(is);
	}
}
