	...
	: m_world(world)
{
	...
	m_buttons.emplace_back("Constitution (+20 HP, from " + std::to_string(player->getMaxHp()) + ")");
	m_buttons.emplace_back("Strength (+1 attack, from " + std::to_string(player->getAttack()) + ")");
	m_buttons.emplace_back("Agility (+1 defense, from " + std::to_string(player->getDefense()) + ")");
}
