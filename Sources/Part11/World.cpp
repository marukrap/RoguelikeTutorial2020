...
void World::createLevel(unsigned int seed, bool placePlayerActor)
{
	auto level = std::make_unique<Level>();
	Actor* actor = level->createMap(m_mapWidth, m_mapHeight, seed, placePlayerActor);
	if (actor)
		m_player = actor;
...
{
	m_menu = std::make_unique<LevelUpMenu>(*this);
}
	...
	if (m_gameState == GameState::EnemyTurn)
	{
		...
		for (auto& actor : *m_actors)
		{
			...
			if (m_player->isDestroyed())
			{
				...
				m_player = nullptr;
				m_menu = nullptr; // Close level up menu if you leveled up this turn.
